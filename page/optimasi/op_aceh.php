<?php
 ...
 $query = mysqli_query($conn, "SELECT * FROM dataaceh ORDER BY rand() ASC LIMIT 3140");
 $quer = mysqli_query($conn, "SELECT dataaceh.*, datapaceh.Namap, datapaceh.Umurp
                            FROM dataaceh, datapaceh
                            WHERE dataaceh.id = datapaceh.pid
                            ORDER BY rand() ASC LIMIT 3970");
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <?php foreach($query as $row){?>
                            <tr>
                                <td>
                                <img src="asset/no-image.png" width="88" class="img-responsive" /></td>
                                <td><?=$row['Nama'];?></td>
                                <td><?=$row['Umur'];?></td>
                        ...
                        $jumlah = 3011 - 30;
                        if($no < $jumlah) {
                        $a = mysqli_query($conn, "SELECT * FROM dataaceh where id = '$myid' ");
                        $b = mysqli_fetch_array($a);
                            mysqli_query($conn, "insert into kuotaaceh values ('$myid', '$b[Nama]', '$b[Umur]', '$b[Status_Menikah]', '$b[Pekerjaan]', '$b[Kota]', '$b[Provinsi]', '$b[Negara]', '$b[Record_Haji]', '$b[Riwayat_Penyakit]', '$b[Rentang_Waktu_Pendaftaran]', '$b[Status]')") or die(mysqli_error($conn));
                        }

                        header("location: index.php?lapaceh&success");
