<?php
 ...
 $query = mysqli_query($conn, "SELECT * FROM datasumaterabarat ORDER BY rand() ASC LIMIT 3599");
            ...
            <div class="panel-body">
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        $jumlah = 3251 - 33;
                        if($no < $jumlah) {
                        $a = mysqli_query($conn, "SELECT * FROM sumaterabarat where id = '$myid' ");
                        $b = mysqli_fetch_array($a);
                            mysqli_query($conn, "insert into kuotasumaterabarat values ('$myid', '$b[Nama]', '$b[Umur]', '$b[Status_Menikah]', '$b[Pekerjaan]', '$b[Kota]', '$b[Provinsi]', '$b[Negara]', '$b[Record_Haji]', '$b[Riwayat_Penyakit]', '$b[Rentang_Waktu_Pendaftaran]', '$b[Status]')") or die(mysqli_error($conn));
                        }
                        header("location: index.php?lapsumatrabarat&success");
