<?php
 ...
 $query = mysqli_query($conn, "SELECT * FROM datakepulauanbangkabelitung ORDER BY rand() ASC LIMIT 732");
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <?php foreach($query as $row){?>
                            <tr>
                                <td>
                                <img src="asset/no-image" width="88" class="img-responsive" /></td>
                                <td><?=$row['Nama'];?></td>
                                <td><?=$row['Umur'];?></td>
                        ...
                        $jumlah = 650 - 9;
                        if($no < $jumlah) {
                        $a = mysqli_query($conn, "SELECT * FROM datakepulauanbangkabelitung where id = '$myid' ");
                        $b = mysqli_fetch_array($a);
                            mysqli_query($conn, "insert into kuotakepulauanbangkabelitung values ('$myid', '$b[Nama]', '$b[Umur]', '$b[Status_Menikah]', '$b[Pekerjaan]', '$b[Kota]', '$b[Provinsi]', '$b[Negara]', '$b[Record_Haji]', '$b[Riwayat_Penyakit]', '$b[Rentang_Waktu_Pendaftaran]', '$b[Status]')") or die(mysqli_error($conn));
                        }
                        header("location: index.php?lapbangka&success");
