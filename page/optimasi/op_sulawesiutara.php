<?php
 ...
 $query = mysqli_query($conn, "SELECT * FROM datasulawesiutara ORDER BY rand() ASC LIMIT 561");
            ...
            <div class="panel-body">
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        $jumlah = 526 - 5;
                        if($no < 526 - 5) {
                        $a = mysqli_query($conn, "SELECT * FROM datasulawesiutara where id = '$myid' ");
                        $b = mysqli_fetch_array($a);
                            mysqli_query($conn, "insert into kuotasulawesiutara values ('$myid', '$b[Nama]', '$b[Umur]', '$b[Status_Menikah]', '$b[Pekerjaan]', '$b[Kota]', '$b[Provinsi]', '$b[Negara]', '$b[Record_Haji]', '$b[Riwayat_Penyakit]', '$b[Rentang_Waktu_Pendaftaran]', '$b[Status]')") or die(mysqli_error($conn));
                        }
                        header("location: index.php?lapsulewesiutara&success");
